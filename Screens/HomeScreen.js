...
class HomeScreen extends React.Component {
  ...
  componentDidMount = async () => {
    ...
    this.props.loadBooks(ComplaintsArray);
    console.log(this.props.allArray);
  };
  ...
  newEntry = () => {
    if (this.state.showTextField == false) {
    ...
    return (
      <View style={{ flex: 1 }}>
        ...
        <View style={{ flex: 1, backgroundColor: "white" }}>
          ...
          <FlatList
            data={this.props.allArray.activeArray}
            keyExtractor={(item, index) => index.toString()}
            renderItem={({ item, index }) => this.renderItem(item, index)}
            ListEmptyComponent={
              <View style={{ alignItems: "center" }}>
                <Text
                  style={{ fontWeight: "bold", fontSize: 18, paddingTop: 40 }}
                >
                  No Records Found
                </Text>
              </View>
